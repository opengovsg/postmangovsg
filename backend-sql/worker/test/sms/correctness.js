...
describe('Basic flow - one campaign, one credential', () => {
  ...
  describe('db runner', () => {
    ...
    it('should contain 10 messages to be sent', async () => {
      const [rows] = await connection.query('SELECT COUNT(*) FROM sms_messages;')
  ...
  describe('worker', () => {
    ...
    it('should enqueue messages', async () => {
      ...
      const smsMessages = await connection.query('SELECT dequeued_at FROM sms_messages WHERE campaign_id=:campaignId;',
        { replacements: { campaignId: currentCampaign }, type: QueryTypes.SELECT },
      )
      expect(smsMessages.length).to.equal(10)
      ...
      const [smsOps] = await connection.query('SELECT campaign_id FROM sms_ops;')
      expect(smsOps.length).to.equal(10)
      smsOps.forEach(smsOp => {
        expect(smsOp.campaign_id).to.equal(currentCampaign)
      })
    })
    it('should get messages to send', async () => {
      ...
      const smsOps = await connection.query('SELECT sent_at FROM sms_ops WHERE campaign_id=:campaignId;',
        { replacements: { campaignId: currentCampaign }, type: QueryTypes.SELECT },
      )
      smsOps.forEach(smsOp => {
        expect(smsOp.sent_at).to.not.be.null
      })
    ...
    it('should send a message', async () => {
      ...
      const [smsOps] = await connection.query('SELECT delivered_at, message_id FROM sms_ops;')
      smsOps.forEach(smsOp => {
        expect(smsOp.delivered_at).to.not.be.null
        expect(smsOp.message_id).to.not.be.null
      })
    })
    it('should log messages back to ground truth', async () => {
      const [expectedUpdate] = await connection.query('SELECT recipient, delivered_at, message_id, error_code FROM sms_ops;')
      ...
      // Make sure ground truth table has whatever was inside sms_ops
      const smsMessages = await connection.query('SELECT recipient, delivered_at, message_id, error_code FROM sms_messages WHERE campaign_id=:campaignId;',
        { replacements: { campaignId: currentCampaign }, type: QueryTypes.SELECT },
      )
      smsMessages.forEach(message => {
        const smsOp = expectedUpdate.find(x => x.recipient === message.recipient)
        expect(message.delivered_at).to.equalDate(smsOp.delivered_at)
        expect(message.message_id).to.equal(smsOp.message_id)
        expect(message.error_code).to.equal(smsOp.error_code)
      })

      // Make sure sms ops has been cleared
      const [smsOps] = await connection.query('SELECT campaign_id FROM sms_ops;')
      expect(smsOps.length).to.equal(0)
    })
  })
})
...
describe('Stop and retry the same campaign', () => {
  ...
  it('should finalize a stopped campaign', async () => {
    ...
    smsMessages.forEach(message => {
      const smsOp = expectedUpdate.find(x => x.recipient === message.recipient)
      try {
        smsOp.delivered_at === null
        // Stopped message
          ? expect(message.delivered_at).to.be.null
        // Sent message
          : expect(message.delivered_at).to.equalDate(smsOp.delivered_at)
      } catch (err) {
        console.log(message, smsOp)
        throw err
      }
  ...
  it('should retry a campaign', async () => {
    ...
    expect(smsOps.length).to.equal(5)

    // Check that the currently enqueued messages were not part of the messages that were sent
    const recipients = new Set(smsOps.map(smsOp => smsOp.recipient))
...
describe('Campaigns competing for the same credential', () => {
  ...
  describe('Multiple jobs for same campaign id', async () => {
    ...
    it('should allow messages to be enqueued by multiple workers for different jobs with the same campaign id', async () => {
      ...
      const smsOps = await connection.query('SELECT recipient FROM sms_ops WHERE campaign_id=:campaignId;',
        { replacements: { campaignId: selectedCampaignId }, type: QueryTypes.SELECT },
      )
      // Check that there are no duplicate recipients
      const uniqRecipients = new Set(smsOps.map(smsOp => smsOp.recipient))
      expect(uniqRecipients.size).to.equal(smsOps.length)
    })
    ...
    const dbConfig =
              {
                ...
                dir: '../../sql/functions',
                numEmailCreds: 0,
                numSmsCreds: 2,// Two different credentials
