...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.createFunction(
        ...
        FROM whatsapp_ops p WHERE
        m.id = p.id;

        DELETE FROM whatsapp_ops p WHERE  p.campaign_id = selected_campaign_id;

        PERFORM update_stats_whatsapp(selected_campaign_id);
      `,
        [],
        { force: true }
    )
  ...
  down: async (queryInterface, Sequelize) => {
    await queryInterface.dropFunction('log_job_whatsapp', [
