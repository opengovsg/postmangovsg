...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.createFunction(
        ...
        WITH logged_jobs AS ( 
          ...
          WHERE campaign_id = ( SELECT q1.campaign_id
            ...
            (
              (
                  NOT EXISTS 
                    ( SELECT 1 FROM whatsapp_ops p WHERE p.campaign_id = q1.campaign_id AND sent_at IS NOT NULL AND delivered_at IS NULL LIMIT 1 )
              )
              OR
              (
                -- 20 seconds has passed since the most recent sent_at (the campaign has been stuck for a while in ops table)
                (SELECT EXTRACT(EPOCH FROM (clock_timestamp()-MAX(p.sent_at))) FROM whatsapp_ops p WHERE p.campaign_id = q1.campaign_id) > 20 
              )
            )
        ...
        IF selected_campaign_id IS NOT NULL THEN
          PERFORM log_job_whatsapp(selected_campaign_id);
        END IF;
      `,
        [],
        { force: true }
  ...
  down: async (queryInterface, Sequelize) => {
    await queryInterface.dropFunction('log_next_job_whatsapp', [
      { type: 'integer', name: 'selected_campaign_id', direction: 'OUT' },
    ])
  }
};
