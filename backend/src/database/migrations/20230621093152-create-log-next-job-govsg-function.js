        ...
        WITH logged_jobs AS ( 
          ...
          WHERE campaign_id = ( SELECT q1.campaign_id
            ...
            (
              (
                  NOT EXISTS 
                    ( SELECT 1 FROM govsg_ops p WHERE p.campaign_id = q1.campaign_id AND sent_at IS NOT NULL AND delivered_at IS NULL LIMIT 1 )
              )
              OR
              (
                -- 20 seconds has passed since the most recent sent_at (the campaign has been stuck for a while in ops table)
                (SELECT EXTRACT(EPOCH FROM (clock_timestamp()-MAX(p.sent_at))) FROM govsg_ops p WHERE p.campaign_id = q1.campaign_id) > 20 
              )
            )
