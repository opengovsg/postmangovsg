...
export const INVALID_FROM_ADDRESS_ERROR_MESSAGE =
  "Invalid 'from' email address, which must be either the default acknowledgement@mail.postman.gov.sg or the user's email (which requires setup with Postman team). Contact us to learn more."
...
export const InitEmailMiddleware = (
  ...
  const isFromAddressAccepted = async (
    ...
    if (
      //  user enters an email that is neither their own nor acknowledgement@mail.postman.gov.sg
   ...
   * - if it is the default acknowledgement@mail.postman.gov.sg, return immediately
