...
- `backend/src/core/config/staging.ts` when `ENVIRONMENT=staging`
- `backend/src/core/config/production.ts` when `ENVIRONMENT=production`
- `backend/src/core/config/default.ts` for any other environment
...
Further reference: https://sequelize.org/master/manual/connection-pool.html
...
|Name|Description|
|--|--|
|AWS_REGION|The region for the S3 bucket that is used to store file uploads|
...
|COOKIE_SECURE| `true` will set a secure cookie that sent only over HTTPS. |
...
Further reference: https://www.npmjs.com/package/express-session

#### OTP and API Keys
|Name|Description|
|--|--|
|OTP_RETRIES|Number of attempts a user can attempt otp before a new otp is required|
...
Further reference: https://docs.aws.amazon.com/ses/latest/DeveloperGuide/smtp-credentials.html
...
Further reference: https://www.twilio.com/docs/iam/keys/api-key-resource?code-sample=code-authenticate-with-api-key-and-api-secret
