...
const Credentials = ({ creds, refresh }: { creds: UserCredential[]; refresh: Function }) => {
  const modalContext = useContext(ModalContext)

  function onAddCredentialClicked() {
    modalContext.setModalContent(<AddCredentialModal labels={creds.map(c => c.label)} onSuccess={refresh}></AddCredentialModal>)
  }
      ...
      <>
        {
          creds.map(({ label, type }) => (
            <tr key={label}>
              <td className='xs'><i className={cx('bx', styles.icon, channelIcons[type])}></i></td>
              <td className='md'>{label}</td>
              <td className={cx('sm', styles.actionColumn)}><i className={cx('bx', 'bx-trash', styles.icon, styles.deleteButton)} onClick={() => onDeleteCredClicked(label)}></i></td>
            </tr>
          ))
        }
      </>
  ...
  return (
    <>
      <div className={styles.credHeader}>
        <h2>Credentials</h2>
        <PrimaryButton className={styles.blueButton} onClick={onAddCredentialClicked}>
          Add credentials
