...
export function exceedsCharacterThreshold(body: string): boolean {
  // replace <br /> new lines with \n as how it would be sent to twilio
  const smsTemplate = (body || '').replace(/<br\s*\/?>/g, '\n')
  // template the body with empty params to extract out the template only
  const extractedTemplate = templateClient.template(smsTemplate, {})
  return extractedTemplate.length > SMS_WARN_EXCEED_CHARACTER_THRESHOLD
}
