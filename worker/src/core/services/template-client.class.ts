  ...
  parseTemplate(
    ...
    try {
            ...
            if (key !== undefined) {
              ...
              if (!keyHasValidChars) {
                throw new TemplateError(
                  `Invalid characters in param named {{${key}}}. Only alphanumeric characters allowed.`
                )
              }
              ...
              // I have not found an edge case that trips this yet
              logger.error(
      ...
      if (err.message.includes('unclosed tag'))
        throw new TemplateError(
          'There are unclosed curly brackets in the template'
        )
